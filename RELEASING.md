...
1. Run `Release Wheels` by hand with `publish=no` and `torch_abis` set to a
   ...
   both wheels must be under the limit (a single-ABI torch wheel is the
   smallest torch wheel the build makes).
2. Run it again with `publish=yes` and the same inputs. This publishes a
   `.dev` pre-release of both packages, which creates the PyPI projects.
...
libtorch symbols. The build needs no torch_tpu checkout: the stand-in module
under [bazel/torch_tpu_standin](bazel/torch_tpu_standin) carries the
header (with the torch_tpu commit it came from) and the XLA revision the
extension must share with torch_tpu. The wheel ships one extension variant
...
- [torch_tpu.version](torch_tpu.version) at the repo root names that
  torch_tpu wheel version. Update it, together with the stand-in's header and
  XLA pin when torch_tpu moved them, before tagging.
