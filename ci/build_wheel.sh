...
# script directly inside a job container. By default no torch_tpu checkout
# is involved: build.sh resolves the torch_tpu module from
# shims/torch_tpu, backed by the torch_tpu wheel the script
# installs. With TORCH_TPU_SRC set to a torch_tpu checkout (torch_tpu's own
# Kokoro job sets it), the torch wheel is built against that checkout
# instead.
#
...
DOCKER_ENV=()
if [[ "${BUILD_MODE}" == "torch" && -n "${TORCH_TPU_SRC}" ]]; then
  if [[ ! -f "${TORCH_TPU_SRC}/MODULE.bazel" ]]; then
    echo "ERROR: TORCH_TPU_SRC=${TORCH_TPU_SRC} is not a torch_tpu checkout" >&2
    exit 1
  fi
  ...
  DOCKER_MOUNTS+=(-v "${TORCH_TPU_SRC}:/torch_tpu")
  DOCKER_ENV+=(-e TORCH_TPU_CHECKOUT=/torch_tpu)
fi
