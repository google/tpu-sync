...
class KVCacheStoreE2ETest(parameterized.TestCase):
  ...
  def _wait_for_save_failure(
    ...
    """Polls until save reports a failure."""
    start = time.time()
    while time.time() - start < timeout_s:
      _, failed, _, _, _ = store.poll_save_status()
      if failed:
        return failed
      time.sleep(0.01)
    raise TimeoutError("Timed out waiting for expected save failure")
  ...
  def _run_remote_read_e2e_test(
    ...
    try:
      ...
      # 9. Verify byte-exact match on Job B TPU device, 2 and 3 stay as created.
      try:
        torch.tpu.synchronize()
      except (AttributeError, RuntimeError):
        pass
  ...
  def _run_remote_write_e2e_test(
    ...
    try:
      ...
      if not expect_write_success:
        failed = self._wait_for_save_failure(store_a)
  ...
  @parameterized.named_parameters(
      ("matching_node_id", 0, 0, True),
      ("mismatched_node_id_fails", 0, 1, False),
  )
