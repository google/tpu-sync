...
# Mirrors _TOPOLOGY_BY_TPU_PCI_DEVICE_ID in
# third_party/py/torch_tpu/_internal/utils/hardware.py
_V4_TOPOLOGY = {1: "1,1,1", 2: "1,2,1", 4: "2,2,1", 8: "2,2,2"}
_V5P_TOPOLOGY = {1: "1,1,1", 2: "1,2,1", 4: "2,2,1", 8: "2,2,2"}
# 8-chip v5e is discovered as a 2x2x2 cube internally, but Cloud TPU VMs wire it
# as a 2x4 mesh. Set TORCH_TPU_TOPOLOGY=2,4,1 to override there.
_V5E_TOPOLOGY = {1: "1,1,1", 4: "2,2,1", 8: "2,2,2"}
_V6E_TOPOLOGY = {1: "1,1,1", 4: "2,2,1", 8: "2,4,1"}
_V7_TOPOLOGY = {2: "1,1,1,2", 4: "1,2,1,2", 8: "2,2,1,2"}
_V8I_TOPOLOGY = {2: "1,1,1,2", 4: "2,1,1,2", 8: "4,1,1,2"}

_TOPOLOGY_BY_TPU_PCI_DEVICE_ID = {
    "0x005e": _V4_TOPOLOGY,
    "0x0062": _V5P_TOPOLOGY,
    "0x0063": _V5E_TOPOLOGY,
    "0x006f": _V6E_TOPOLOGY,
    "0x0076": _V7_TOPOLOGY,
    "0x0083": _V8I_TOPOLOGY,
}
...
def get_tpu_topology(world_size: int) -> str:
  """Chip mesh for world_size devices on the attached TPU generation."""
  _, topology_map = _scan_pci_tpus()
  if topology_map is None:
    raise RuntimeError("No TPU devices found.")
  if world_size not in topology_map:
    raise RuntimeError(
        f"No TPU topology for {world_size} chips on this host. Set"
        " TORCH_TPU_TOPOLOGY to override."
    )
