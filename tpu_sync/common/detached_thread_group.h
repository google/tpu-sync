...
class DetachedThreadGroup final {
 public:
  ...
  // Runs |task| on a new detached thread. Crashes if the thread cannot be
  // started: that only happens when the process is out of threads, memory or
  // virtual memory areas, and silently shedding work would hide a defect that
  // no ordinary alerting catches.
  void Spawn(absl::AnyInvocable<void() &&> task);
