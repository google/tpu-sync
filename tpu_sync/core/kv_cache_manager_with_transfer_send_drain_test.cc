...
class RecvTestManager : public KVCacheManagerWithTransfer {
 public:
  ...
  void AddRecv(const std::string& req_id, uint64_t uuid,
    ...
    absl::MutexLock lock(mu_);
    auto entry = std::make_shared<RecvEntry>(base(), uuid);
    entry->set_req_id(req_id);
    entry->set_slot_idx(AcquireSlotLocked().slot_idx);
    entry->set_total_blocks(blocks_per_layer);
    entry->set_deadline(DeadlineFromNow());
    entry->set_start_time(std::chrono::steady_clock::now());
