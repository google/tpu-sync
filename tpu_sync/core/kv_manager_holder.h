...
template <typename T, typename = void>
struct has_register_kv_backends : std::false_type {};

template <typename T>
struct has_register_kv_backends<
    T, std::void_t<decltype(std::declval<T&>().RegisterKVBackends(
           std::declval<absl::Span<const kv_cache::BackendConfig>>()))>>
    : std::true_type {};

template <typename T>
inline constexpr bool has_register_kv_backends_v =
    has_register_kv_backends<T>::value;
 ...
 public:
  class Concept {
   public:
    ...
    virtual std::vector<kv_cache::backends::HostBufferDescriptor>
  ...
  class Model final : public Concept {
   public:
    ...
    std::vector<kv_cache::backends::HostBufferDescriptor> ResolveBlockSlices(
  ...
  void RegisterKVBackends(
    ...
    if (self_) {
      self_->RegisterKVBackends(configs);
    }
  }
