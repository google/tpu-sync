...
bool TransferReceiveSession::EndOpLocked() {
  if (in_flight_ <= 0) {
    LOG(DFATAL) << "Receive operation count underflow for UUID " << uuid_;
    return false;
  }
  --in_flight_;
  return draining_ && in_flight_ == 0;
}

bool TransferReceiveSession::FinishLocked(bool has_failed) {
  if (is_pool_reshard_) return false;
...
bool TransferReceiveSession::AllH2dDone() const {
...
bool TransferReceiveSession::IsReadyToComplete() const {
  ...
  return (network_completed_ || num_completed_layers_ == total_layers) &&
         AllH2dDone();
}
...
void TransferReceiveSession::ExecutePullRequest(
    ...
    CopyPlan load_plan) {
  std::shared_ptr<TransferReceiveSession> self = shared_from_this();
  std::optional<int> target_node = base_->assigned_numa_node();

  base_->push_pool()->Schedule(target_node, [&manager, self, remote_endpoint,
                                             load_plan =
                                                 std::move(load_plan)]() {
    bool pull_failed = false;
    try {
      LOG(INFO) << "StartRead (connecting): req_id=" << self->req_id_
                << ", uuid=" << self->uuid_
    ...
    std::optional<uint64_t> plan_gen;
    {
      absl::MutexLock lock(manager.mu_);
      if (pull_failed) {
        plan_gen = manager.FinishRecvLocked(self, /*failed=*/true);
      }
      std::optional<uint64_t> ended = manager.EndRecvOpLocked(self);
      if (ended.has_value()) {
        plan_gen = ended;
      }
    }
    if (plan_gen.has_value()) {
      manager.UnregisterSettledPlan(self->uuid_, *plan_gen);
    }
  });
}
...
absl::Status TransferReceiveSession::OnBlocksReceived(
  ...
  {
    absl::MutexLock lock(manager.mu_);
    auto it = manager.active_recv_entries_.find(uuid_);
    if (it == manager.active_recv_entries_.end() || it->second.get() != this) {
      return absl::OkStatus();
    }
    if (is_pool_reshard_) {
      return absl::OkStatus();
    }
    bool all_complete = RecordBlocksReceivedLocked(block_ids, &first_packet,
                                                   &network_just_completed);
    if (first_packet && manager.metrics_collector_) {
      manager.metrics_collector_->RecordFirstPacket(uuid_);
    }
    ...
    if (manager.metrics_collector_) {
      manager.metrics_collector_->RecordLastPacket(uuid_);
    }
    if (all_complete) {
      session_start_time = start_time_;
      should_record_duration = true;
      if (manager.metrics_collector_) {
        manager.metrics_collector_->RecordEnd(uuid_);
      }
      plan_gen = manager.FinishRecvLocked(shared_from_this(), /*failed=*/false);
    }
  }
  ...
  if (plan_gen.has_value()) {
    manager.UnregisterSettledPlan(uuid_, *plan_gen);
  }

  if (should_record_duration) {
    LOG(INFO) << "OnBlocksReceived (Network + H2D complete): req_id="
              << session_req_id << ", uuid=" << uuid_
...
absl::Status TransferReceiveSession::ExecuteLayerH2d(
  ...
  future.OnReady([&manager, self, layer_idx, session_req_id,
                  metrics_collector =
                      manager.metrics_collector_](auto status_or) {
    std::optional<uint64_t> plan_gen;
    {
      absl::MutexLock lock(manager.mu_);
      auto it = manager.active_recv_entries_.find(self->uuid_);
      if (it == manager.active_recv_entries_.end() || it->second != self) {
        LOG(DFATAL) << "H2D callback for retired receive UUID " << self->uuid_;
        return;
      }
      if (status_or.ok()) {
        LOG(INFO) << "OnLayerReceived (H2D copy complete) layer " << layer_idx
                  << ": req_id=" << session_req_id << ", numa="
                  << self->base_->assigned_numa_node().value_or(-1);
        self->num_completed_layers_++;
        if (self->num_completed_layers_ == self->base_->num_layers() &&
            !self->draining_) {
          RecordTransferDuration(
              DurationMs(self->start_time_, std::chrono::steady_clock::now()));

          if (metrics_collector) {
            metrics_collector->RecordH2dComplete(self->uuid_);
          }
          LOG(INFO) << "All layers H2D copy complete: req_id="
                    << session_req_id;
          if (metrics_collector) {
            metrics_collector->RecordEnd(self->uuid_);
          }
          plan_gen = manager.FinishRecvLocked(self, /*failed=*/false);
        }
      } else {
...
void TransferReceiveSession::ExecuteEligiblePoolH2ds(
  ...
  for (auto& [pool_idx, dst_chip_block_ids] : to_launch) {
    ...
    future.OnReady([&manager, self, pool_idx = pool_idx](auto status_or) {
      self->FinishPoolH2d(
          manager, pool_idx,
...
void TransferReceiveSession::FinishPoolH2d(KVCacheManagerWithTransfer& manager,
  ...
  if (finished) {
    ...
    if (!status.ok() || (!unregister.ok() && !absl::IsNotFound(unregister))) {
      manager.failed_recving_.insert(req_id_);
      manager.active_recv_entries_.erase(it);
    } else {
      session_start_time = start_time_;
      should_record_duration = true;
      network_completed_ = true;
      manager.done_recving_.insert(req_id_);
    }
  }
  if (should_record_duration) {
    RecordTransferDuration(
