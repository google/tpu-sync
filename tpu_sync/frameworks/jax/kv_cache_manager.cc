...
NumaAwareKVCacheManager::D2hWriteToBackend(
  ...
  for (auto& sub_mgr : sub_managers_) {
    if (sub_mgr != nullptr && sub_mgr->base() != nullptr) {
      ABSL_ASSIGN_OR_RETURN(auto fut, sub_mgr->base()->D2hWriteToBackend(
                                          backends, block_keys,
                                          src_device_block_ids,
                                          dst_host_block_ids));
...
NumaAwareKVCacheManager::H2dReadFromBackend(
  ...
  for (auto& sub_mgr : sub_managers_) {
    if (sub_mgr != nullptr && sub_mgr->base() != nullptr) {
      ABSL_ASSIGN_OR_RETURN(auto fut, sub_mgr->base()->H2dReadFromBackend(
                                          backends, block_keys,
                                          src_host_block_ids,
...
NumaAwareKVCacheManager::GetKVBackend(absl::string_view backend_name) const {
  for (const auto& sub_mgr : sub_managers_) {
    if (sub_mgr != nullptr && sub_mgr->base() != nullptr) {
      auto backend = sub_mgr->base()->GetKVBackend(backend_name);
      if (backend != nullptr) {
        return backend;
      }
    }
