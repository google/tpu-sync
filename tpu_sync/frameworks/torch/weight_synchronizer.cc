...
NumaAwareWeightSynchronizer::NumaAwareWeightSynchronizer(
  ...
  size_t num_subs = 1;
  if (!force_single_numa && total_num_shards_ > 1) {
    num_subs = std::min(total_num_shards_, std::max<size_t>(2, num_ext_nics));
  }

  global_shard_to_submanager_.resize(total_num_shards_);
  submanager_to_global_shards_.resize(num_subs);
  submanager_to_local_shards_.resize(num_subs);

  size_t shards_per_sub = (total_num_shards_ + num_subs - 1) / num_subs;

  for (size_t s = 0; s < num_subs; ++s) {
    size_t start_shard = s * shards_per_sub;
    if (start_shard >= total_num_shards_) break;
    size_t end_shard =
        std::min(start_shard + shards_per_sub, total_num_shards_);
    size_t sub_shards = end_shard - start_shard;

    std::optional<int> sub_port = local_port;
    if (sub_port.has_value() && *sub_port > 0) {
      sub_port = *sub_port + static_cast<int>(s);
    }
    std::optional<int> sub_listener_port = listener_port;
    if (sub_listener_port.has_value() && *sub_listener_port > 0) {
      sub_listener_port = *sub_listener_port + static_cast<int>(s);
    }
    ...
    sub->SetGlobalShardIndices(submanager_to_global_shards_[s]);
    sub->SetLocalShardIndices(submanager_to_local_shards_[s]);
    if (s == 0) {
      sub->SetControlDelegate(this);
    }
...
void NumaAwareWeightSynchronizer::InitSubManagers(
  ...
  if (force_single_numa) {
    ...
    if (total_num_shards_ > 1) {
      ...
      // In multi-process SPMD (e.g. PyTorch torchrun), each process manages
      // a single local shard (total_num_shards_ == 1). Map each process rank
      // to its corresponding NUMA node / data NIC.
      const char* local_rank_env = std::getenv("LOCAL_RANK");
      if (local_rank_env == nullptr) {
        local_rank_env = std::getenv("RANK");
      }
      const char* local_world_size_env = std::getenv("LOCAL_WORLD_SIZE");
      if (local_world_size_env == nullptr) {
        local_world_size_env = std::getenv("WORLD_SIZE");
      }
      if (local_rank_env != nullptr) {
        int local_rank = 0;
        int local_world_size = static_cast<int>(num_ext_nics);
        if (absl::SimpleAtoi(local_rank_env, &local_rank)) {
          if (local_world_size_env != nullptr) {
            (void)absl::SimpleAtoi(local_world_size_env, &local_world_size);
          }
          if (local_world_size <= 0) local_world_size = num_ext_nics;
          size_t nic_idx = (local_rank * num_ext_nics) / local_world_size;
          if (nic_idx >= num_ext_nics) nic_idx = num_ext_nics - 1;
          int target_numa = data_nics[nic_idx].numa_node;
          numa_to_shards.clear();
          numa_to_shards[target_numa].push_back(0);
        }
      }
    }
  }
  ...
  for (int attempt = 0; attempt < kMaxPortAttempts; ++attempt) {
    ...
    for (const auto& [numa, shards] : numa_to_shards) {
      ...
      LOG(INFO) << "[NumaAwareWeightSynchronizer] Submanager " << sub_idx
                << " for NUMA " << numa << " binding to data NIC IP: "
                << sub_bind_ip.value_or("default");
...
absl::Status NumaAwareWeightSynchronizer::RegisterExpectedChunks(
  ...
  for (size_t s = 0; s < sub_synchronizers_.size(); ++s) {
    if (sub_synchronizers_[s]) {
      RETURN_IF_ERROR(sub_synchronizers_[s]->RegisterExpectedChunksLocal(
          uuid, sub_totals[s]));
    }
  }
  return absl::OkStatus();
...
absl::Status NumaAwareWeightSynchronizer::WaitForTransferCompletion(
  ...
  for (auto& sub : sub_synchronizers_) {
    if (sub) {
      RETURN_IF_ERROR(sub->WaitForTransferCompletion(uuid));
    }
  }
  return absl::OkStatus();
}
...
absl::Status WeightSynchronizer::PushWeights(
    const std::vector<std::string>& peers) {
  return numa_manager_->PushWeights(peers);
}

absl::Status WeightSynchronizer::PushWeightsResharded(
    const tpu_sync::rpc::StartTransferRequest& request) {
  return numa_manager_->PushWeightsResharded(request);
}
...
void WeightSynchronizer::DrainPendingH2d() {
