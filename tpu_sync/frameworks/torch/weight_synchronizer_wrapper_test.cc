...
class MockSubWeightSynchronizer : public weight_sync::WeightSynchronizerBase {
 public:
  ...
  bool fail_push_resharded = false;
  std::vector<std::string> last_push_peers;

  int skip_tiling_bool_calls = 0;
  int skip_tiling_vec_calls = 0;

  void SetSkipTiling(bool skip_all) override {
    skip_tiling_bool_calls++;
    weight_sync::WeightSynchronizerBase::SetSkipTiling(skip_all);
  }

  void SetSkipTiling(const std::vector<bool>& skip_tiling) override {
    skip_tiling_vec_calls++;
    weight_sync::WeightSynchronizerBase::SetSkipTiling(skip_tiling);
  }
  ...
  absl::Status PushWeightsResharded(
    ...
    if (fail_push_resharded) {
      return absl::InternalError("Simulated PushWeightsResharded failure");
    }
    return absl::OkStatus();
  }
...
TEST(WeightSynchronizerWrapperTest, PushWeightsDispatchesToAllSubs) {
  auto sub0_raw = new MockSubWeightSynchronizer(2, 4, 1024);
  auto sub1_raw = new MockSubWeightSynchronizer(2, 4, 1024);
  ...
  std::vector<std::string> peers = {"127.0.0.1:10001", "127.0.0.1:10002"};
  absl::Status status = ws.PushWeights(peers);
  EXPECT_TRUE(status.ok());
  EXPECT_EQ(sub0_raw->push_calls, 1);
  EXPECT_EQ(sub1_raw->push_calls, 1);
  EXPECT_EQ(sub0_raw->last_push_peers, peers);
  EXPECT_EQ(sub1_raw->last_push_peers, peers);
}
...
TEST(WeightSynchronizerWrapperTest, MetadataConstructorEndpointsMapping) {
  WeightSynchronizer ws(2, 4, 1024);
...
TEST(WeightSynchronizerWrapperTest,
  ...
  EXPECT_EQ(
      sub0_raw->last_registered_chunks + sub1_raw->last_registered_chunks, 5);
}

TEST(WeightSynchronizerWrapperTest, ResetMetricsClearsAllSubs) {
  auto sub0_raw = new MockSubWeightSynchronizer(2, 4, 1024);
  auto sub1_raw = new MockSubWeightSynchronizer(2, 4, 1024);
  ...
  ws.ResetMetrics();
  EXPECT_EQ(sub0_raw->reset_metrics_calls, 1);
  EXPECT_EQ(sub1_raw->reset_metrics_calls, 1);
}

TEST(WeightSynchronizerWrapperTest, SetSkipTilingDispatchesToAllSubs) {
  auto sub0_raw = new MockSubWeightSynchronizer(2, 4, 1024);
  auto sub1_raw = new MockSubWeightSynchronizer(2, 4, 1024);
  ...
  ws.SetSkipTiling(true);
  EXPECT_EQ(sub0_raw->skip_tiling_bool_calls, 1);
  EXPECT_EQ(sub1_raw->skip_tiling_bool_calls, 1);

  std::vector<bool> skip_vec = {true, false};
...
TEST(WeightSynchronizerWrapperTest, PushWeightsReshardedErrorPropagation) {
  auto sub0_raw = new MockSubWeightSynchronizer(2, 4, 1024);
  auto sub1_raw = new MockSubWeightSynchronizer(2, 4, 1024);
  sub1_raw->fail_push_resharded = true;
  ...
  EXPECT_NE(status.message().find("Simulated PushWeightsResharded failure"),
            std::string::npos);
}

TEST(WeightSynchronizerWrapperTest, EmptySubSynchronizersSafeHandling) {
  std::vector<std::unique_ptr<weight_sync::WeightSynchronizerBase>> empty_subs;
  WeightSynchronizer ws(std::move(empty_subs));

  EXPECT_EQ(ws.num_layers(), 0);
  EXPECT_EQ(ws.num_shards(), 0);
  EXPECT_EQ(ws.slice_byte_size(), 0);
  EXPECT_EQ(ws.GetHostBufferPtr(0, 0), nullptr);

  auto d2h_res = ws.D2h();
  EXPECT_TRUE(d2h_res.ok());

  auto h2d_res = ws.H2d();
  EXPECT_TRUE(h2d_res.ok());

  EXPECT_TRUE(ws.PushWeights({}).ok());

  tpu_sync::rpc::StartTransferRequest req;
  EXPECT_TRUE(ws.PushWeightsResharded(req).ok());

  ws.DrainPendingH2d();
  ws.ResetMetrics();

  auto m = ws.GetMetrics();
  EXPECT_EQ(m.last_d2h_bytes, 0);
}

TEST(WeightSynchronizerWrapperTest, ForgetPushProgressClearsState) {
  auto sub0_raw = new MockSubWeightSynchronizer(2, 4, 1024);
  auto sub1_raw = new MockSubWeightSynchronizer(2, 4, 1024);
  ...
  numa_ws.StoreSkipTiling(77, req);
  numa_ws.ForgetPushProgress(77);

  // Now fallback should occur because schedule counts were cleared
  absl::flat_hash_map<size_t, uint32_t> layer_counts = {{0, 8}};
  EXPECT_TRUE(numa_ws.RegisterExpectedLayerChunks(77, layer_counts).ok());
  EXPECT_EQ(sub0_raw->last_registered_layer_chunks[0], 4);
  EXPECT_EQ(sub1_raw->last_registered_layer_chunks[0], 4);
}
