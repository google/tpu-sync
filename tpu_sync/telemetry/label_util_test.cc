...
TEST(LabelUtilTest, FormatPrometheusLabelsToBufferEmpty) {
  EXPECT_TRUE(PrometheusLabelView({}).view().empty());
...
TEST(LabelUtilTest, FormatPrometheusLabelsToBufferSingleFastPath) {
  MetricLabel labels[] = {{"direction", "pull"}};
  EXPECT_EQ(PrometheusLabelView(labels).view(), "{direction=\"pull\"}");
  ...
  MetricLabel large_labels[] = {{"long_key", long_value}};
  EXPECT_EQ(PrometheusLabelView(large_labels).view(),
            absl::StrCat("{long_key=\"", long_value, "\"}"));
}
...
TEST(LabelUtilTest, FormatPrometheusLabelsToBufferMultiSorted) {
  ...
  EXPECT_EQ(PrometheusLabelView(labels).view(),
  ...
  EXPECT_EQ(
      PrometheusLabelView(ten_labels).view(),
...
TEST(LabelUtilTest, FormatPrometheusLabelsToBufferEscapingAndUtf8Value) {
  ...
  EXPECT_EQ(PrometheusLabelView(utf8_labels).view(),
            "{error_detail=\"失敗: \\\"IO\\\\Error\\\"\\n詳細\"}");
}

TEST(LabelUtilTest, FormatPrometheusLabelsToBufferRawKeys) {
  // Verifies that PrometheusLabelView acts as a raw serializer without
  // enforcing Prometheus key naming restrictions or duplicate key rejection.
  MetricLabel raw_labels[] = {
      {"replica", "1"},
      {"replica", "0"},
  };
  EXPECT_EQ(PrometheusLabelView(raw_labels).view(),
            "{replica=\"0\",replica=\"1\"}");

  MetricLabel special_keys[] = {{"bad-key", "value"}};
  EXPECT_EQ(PrometheusLabelView(special_keys).view(), "{bad-key=\"value\"}");
}
...
TEST(LabelUtilTest, PrometheusLabelViewDefaultCapacityOverflow) {
  ...
  const std::array<MetricLabel, 1> labels = {
      MetricLabel{.key = "key", .value = long_val},
  };
  PrometheusLabelView view(labels);
  std::string expected = absl::StrCat("{key=\"", long_val, "\"}");
  EXPECT_EQ(view.view(), expected);
  std::string owned = std::move(view).ToOwned();
...
TEST(LabelUtilTest, PrometheusLabelViewUnsortedLabelsOverflow) {
  std::string long_val(300, 'x');
  const std::array<MetricLabel, 2> labels = {
      MetricLabel{.key = "z", .value = long_val},
      MetricLabel{.key = "a", .value = "first"},
  };
  PrometheusLabelView view(labels);
  std::string expected = absl::StrCat("{a=\"first\",z=\"", long_val, "\"}");
  EXPECT_EQ(view.view(), expected);
  EXPECT_EQ(std::move(view).ToOwned(), expected);
}
...
TEST(LabelUtilTest, PrometheusLabelViewEscapingOverflow) {
  ...
  std::string expected = absl::StrCat("{q=\"", expected_val, "\"}");
  EXPECT_EQ(view.view(), expected);
  EXPECT_EQ(std::move(view).ToOwned(), expected);
}
...
TEST(LabelUtilTest, PrometheusLabelViewToOwnedStackAndHeap) {
  ...
  const std::array<MetricLabel, 1> heap_labels = {
      MetricLabel{.key = "env", .value = long_val},
  };
  std::string expected = absl::StrCat("{env=\"", long_val, "\"}");
