...
class TelemetryBindingTest(absltest.TestCase):
  ...
  def test_metric_metadata_properties_repr_equality(self):
    ...
    self.assertEqual(
        metrics_by_name["h2d_bytes_total"].type,
        telemetry_ext.MetricType.COUNTER,
    )
    self.assertTrue(metrics_by_name["h2d_bytes_total"].description)
    self.assertIn(
        "label_names=['host_ip', 'local_rank']",
        repr(metrics_by_name["h2d_bytes_total"]),
    )
    ...
    self.assertEqual(
        metrics_by_name["d2h_bytes_total"].type,
        telemetry_ext.MetricType.COUNTER,
    )
    self.assertTrue(metrics_by_name["d2h_bytes_total"].description)
