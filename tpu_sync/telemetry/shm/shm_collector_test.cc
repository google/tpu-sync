...
TEST_F(ShmCollectorTest, AggregatesLiveWorkersAndReapsDead) {
  ...
  w1.ObserveHistogram(metric_names::kTransferDurationMs,
  ...
  std::string labeled_prefix =
      "transfer_duration_ms/direction=push;error_code=RESOURCE_EXHAUSTED";
  EXPECT_THAT(totals[absl::StrCat(labeled_prefix, "/count")], DoubleEq(1.0));
  EXPECT_THAT(totals[absl::StrCat(labeled_prefix, "/sum")], DoubleEq(0.25));
  EXPECT_THAT(totals[absl::StrCat(labeled_prefix, "/bucket_1")], DoubleEq(1.0));
  ...
  for (const auto& entry : std::filesystem::directory_iterator(test_dir_)) {
    ...
    if (absl::StrContains(fn, "worker_rank_dead_")) {
      ++dead_files;
    }
    if (absl::StrContains(fn, "worker_rank_0_") ||
...
TEST_F(ShmCollectorTest, HandlesDeadAndLiveTruncatedFiles) {
  // Dead truncated file (no lock held) must be reaped.
  const std::string dead_trunc_path = absl::StrCat(
      test_dir_, "/", kShmFilePrefix, "dead_trunc", kShmFileExtension);
  {
    std::ofstream ofs(dead_trunc_path, std::ios::binary);
    ofs.write("short", 5);
  }

  // Live truncated file (shared lock held) must be skipped and preserved.
  ...
  EXPECT_FALSE(std::filesystem::exists(dead_trunc_path));
...
TEST_F(ShmCollectorTest, DirectAggregateSegmentEdgeCases) {
  ...
  auto* histogram_slot =
      new (mem->data + pool + 2 * kMetricSlotAlignment) ShmHistogramSlot();
  histogram_slot->sample_count.store(1);
  histogram_slot->sample_sum.store(0.5);
  histogram_slot->bucket_counts[2].store(1);

  const uint32_t nan_hist_pool = pool + 3 * kMetricSlotAlignment;
  auto* nan_hist = new (mem->data + nan_hist_pool) ShmHistogramSlot();
  nan_hist->sample_count.store(10);
  nan_hist->sample_sum.store(std::numeric_limits<double>::quiet_NaN());
  nan_hist->bucket_counts[0].store(10);

  const uint32_t inf_hist_pool = pool + 4 * kMetricSlotAlignment;
  auto* inf_hist = new (mem->data + inf_hist_pool) ShmHistogramSlot();
  inf_hist->sample_count.store(20);
  inf_hist->sample_sum.store(std::numeric_limits<double>::infinity());
  inf_hist->bucket_counts[0].store(20);
  ...
  const std::array specs = {
      ...
      EntrySpec{"hist_m", MetricType::kHistogram,
                ...
                TocEntryState::kCommitted},
      EntrySpec{"nan_hist", MetricType::kHistogram, nan_hist_pool,
                sizeof(ShmHistogramSlot), TocEntryState::kCommitted},
      EntrySpec{"inf_hist", MetricType::kHistogram, inf_hist_pool,
                sizeof(ShmHistogramSlot), TocEntryState::kCommitted},
      ...
      EntrySpec{"corrupt_type", static_cast<MetricType>(99), pool, 8,
                TocEntryState::kCommitted},
  };
  ...
  EXPECT_FALSE(totals.contains("corrupt_type"));
  EXPECT_FALSE(totals.contains("nan_hist/count"));
  EXPECT_FALSE(totals.contains("nan_hist/sum"));
  EXPECT_FALSE(totals.contains("inf_hist/count"));
  EXPECT_FALSE(totals.contains("inf_hist/sum"));
