...
TEST_F(ShmCollectorTest, ResilientToCorruptedFilesAndNonExistentDir) {
  ...
  auto write_hdr = [&](absl::string_view name, uint32_t magic, uint32_t ver) {
    ShmTocHeader h{.version = ver};
    h.magic.store(magic);
  ...
  write_hdr("bad_magic.mmap", 0xDEADBEEF, kSupportedVersion);
  write_hdr("bad_ver.mmap", kRaidenShmMagic, 999);
  ...
  absl::flat_hash_map<std::string, double> totals;
  ShmCollector(CollectorOptions()).CollectMetrics(totals);
  EXPECT_EQ(totals["sent_bytes_total/direction=push"], 500.0);
  EXPECT_TRUE(
...
TEST_F(ShmCollectorTest, DirectAggregateSegmentEdgeCases) {
  ...
  seg->header.version = kSupportedVersion;
  seg->header.max_toc_entries = kMaxTocEntries;
  seg->header.data_pool_offset = sizeof(ShmSegmentLayout);

  // Unaligned segment pointer.
  auto unaligned_seg = reinterpret_cast<const ShmSegmentLayout*>(
      reinterpret_cast<uintptr_t>(seg) + 1);
  ...
  new (mem->data + pool + 64) std::atomic<uint64_t>(100);
  auto* h = reinterpret_cast<ShmHistogramSlot*>(mem->data + pool + 128);
  h->sample_count.store(1);
  h->sample_sum.store(0.5);
  h->bucket_counts[2].store(1);
  ...
  const std::array specs = {
      ...
      EntrySpec{"counter_m", MetricType::kCounter, pool + 64, 8,
                TocEntryState::kCommitted},
      EntrySpec{"hist_m", MetricType::kHistogram, pool + 128,
      ...
      EntrySpec{"unaligned", MetricType::kCounter, pool + 3, 8,
                TocEntryState::kCommitted},
      EntrySpec{"", MetricType::kCounter, pool + 64, 8,
                TocEntryState::kCommitted},
      EntrySpec{"underflow", MetricType::kCounter,
                sizeof(ShmSegmentLayout) - 64, 8, TocEntryState::kCommitted},
  };
  for (size_t i = 0; i < specs.size(); ++i) {
    ShmTocEntry& e = seg->toc[i];
    snprintf(e.metric_name, sizeof(e.metric_name), "%.*s",
             static_cast<int>(specs[i].name.size()), specs[i].name.data());
    e.type = specs[i].type;
    e.offset = specs[i].off;
    e.size = specs[i].size;
    e.entry_state.store(specs[i].state);
  }
