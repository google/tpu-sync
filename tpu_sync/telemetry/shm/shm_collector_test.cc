...
class ShmCollectorTest : public testing::Test {
 protected:
  void SetUp() override {
    test_dir_ = (std::filesystem::path(testing::TempDir()) /
                 absl::StrCat("shm_col_", getpid(), "_",
  ...
  AggregatedMetrics Collect(absl::string_view sub_dir = "") const {
    const std::string dir = sub_dir.empty() ? test_dir_ : FilePath(sub_dir);
    return ShmCollector(ShmCollectorOptions{.shm_dir = dir}).CollectMetrics();
  }

  void CollectInto(AggregatedMetrics& metrics) const {
    metrics = ShmCollector(CollectorOptions()).CollectMetrics();
  }
...
TEST_F(ShmCollectorTest, HandlesNonExistentShmDir) {
  EXPECT_TRUE(Collect("nonexistent").empty());
}
...
TEST_F(ShmCollectorTest, AggregatesLiveWorkersAndReapsDead) {
  ...
  const std::array<MetricLabel, 1> host_pool_labels = {
      MetricLabel{"pool", "host"}};
  const std::array<MetricLabel, 1> device_pool_labels = {
      MetricLabel{"pool", "device"}};
  ...
  writer_0.SetGauge(metric_names::kBufferAllocatedBytes, host_pool_labels,
                    500.0);
  writer_1.SetGauge(metric_names::kBufferAllocatedBytes, host_pool_labels,
                    700.0);
  writer_1.SetGauge(metric_names::kBufferAllocatedBytes, device_pool_labels,
                    250.0);
  ...
  AggregatedMetrics metrics = Collect();

  // Verify Counters
  EXPECT_EQ(metrics.counters[metric_names::kSentBytesTotal]["direction=push"],
            300);
  EXPECT_EQ(metrics.counters[metric_names::kSentBytesTotal]["direction=pull"],
            125);
  EXPECT_EQ(metrics.counters[metric_names::kTransferFailuresTotal]
                            ["direction=pull;error_code=DEADLINE_EXCEEDED"],
            5);

  // Verify Gauges
  EXPECT_DOUBLE_EQ(metrics.gauges[metric_names::kBufferAllocatedBytes][""],
                   3000.0);
  EXPECT_DOUBLE_EQ(
      metrics.gauges[metric_names::kBufferAllocatedBytes]["pool=host"], 1200.0);
  EXPECT_DOUBLE_EQ(
      metrics.gauges[metric_names::kBufferAllocatedBytes]["pool=device"],
      250.0);

  // Verify Histograms (Non-cumulative)
  const HistogramData& hist_unlabeled =
      metrics.histograms[metric_names::kTransferDurationMs][""];
  EXPECT_EQ(hist_unlabeled.sample_count, 2);
  EXPECT_DOUBLE_EQ(hist_unlabeled.sample_sum, 0.20);
  // writer_0 observed 0.05 -> bucket 0 (le=0.1)
  // writer_1 observed 0.15 -> bucket 1 (le=0.25)
  EXPECT_EQ(hist_unlabeled.bucket_counts[0], 1);
  EXPECT_EQ(hist_unlabeled.bucket_counts[1], 1);
  EXPECT_EQ(hist_unlabeled.bucket_counts[2], 0);

  const HistogramData& hist_labeled =
      metrics.histograms[metric_names::kTransferDurationMs]
                        ["direction=push;error_code=RESOURCE_EXHAUSTED"];
  EXPECT_EQ(hist_labeled.sample_count, 1);
  EXPECT_DOUBLE_EQ(hist_labeled.sample_sum, 0.25);
  // writer_1 observed 0.25 -> bucket 1 (le=0.25)
  EXPECT_EQ(hist_labeled.bucket_counts[0], 0);
  EXPECT_EQ(hist_labeled.bucket_counts[1], 1);
...
TEST_F(ShmCollectorTest, ConcurrencyAndProcessCrashHandling) {
  ...
  for (int i = 0; i < 8; ++i) {
    threads.emplace_back([this, &start_notification]() {
      ...
      AggregatedMetrics thread_metrics =
          ShmCollector(CollectorOptions()).CollectMetrics();
      const auto it =
          thread_metrics.counters.find(metric_names::kSentBytesTotal);
      ASSERT_NE(it, thread_metrics.counters.end());
      EXPECT_FALSE(it->second.contains("direction=push"));
      const auto label_it = it->second.find("");
      ASSERT_NE(label_it, it->second.end());
      EXPECT_EQ(label_it->second, 42);
    });
  }
  ...
  AggregatedMetrics final_metrics = Collect();
  const auto it = final_metrics.counters.find(metric_names::kSentBytesTotal);
...
TEST_F(ShmCollectorTest, AggregatesMultiChunkWorker) {
  ...
  AggregatedMetrics metrics = Collect();
  EXPECT_THAT(metrics.counters, SizeIs(kMaxTocEntries + 20));
  EXPECT_EQ(metrics.counters["metric_0"][""], 1);
  EXPECT_EQ(metrics.counters[absl::StrCat("metric_", kMaxTocEntries - 1)][""],
            1);
  EXPECT_EQ(metrics.counters[absl::StrCat("metric_", kMaxTocEntries)][""], 1);
  EXPECT_EQ(metrics.counters[absl::StrCat("metric_", kMaxTocEntries + 19)][""],
            1);
}
...
TEST_F(ShmCollectorTest, AggregatesMultiChunkHistogramsOnPoolExhaustion) {
  ...
  AggregatedMetrics metrics = Collect();
  EXPECT_THAT(metrics.histograms, SizeIs(kNumHistograms));
  EXPECT_EQ(metrics.histograms["hist_0"][""].sample_count, 1);
  EXPECT_EQ(metrics.histograms["hist_0"][""].bucket_counts[0], 1);
  EXPECT_EQ(metrics.histograms[absl::StrCat("hist_", kNumHistograms - 1)][""]
                .sample_count,
            1);
}
...
TEST_F(ShmCollectorTest, ClearsPrePopulatedMetrics) {
  ShmWriter live(WriterOptions("live"));
  live.IncrementCounter(metric_names::kSentBytesTotal, {}, 10);

  AggregatedMetrics metrics;
  metrics.counters["stale_a"][""] = 999;
  metrics.gauges["stale_b"][""] = 123.0;

  CollectInto(metrics);

  EXPECT_THAT(metrics.counters, SizeIs(1));
  EXPECT_EQ(metrics.counters[metric_names::kSentBytesTotal][""], 10);
...
TEST_F(ShmCollectorTest, SkipsDefensiveSlotValidationFailures) {
  ...
  AggregatedMetrics metrics = Collect();
  EXPECT_THAT(metrics.counters, SizeIs(1));
  EXPECT_THAT(metrics.histograms, SizeIs(1));

  EXPECT_EQ(metrics.counters["valid_counter"][""], 100);
  EXPECT_EQ(metrics.histograms["zero_hist"][""].sample_count, 0);

  for (absl::string_view name :
       {"unaligned_counter", "below_pool_offset", "exceeds_filesize",
        "overflow_offset", "undersized_counter", "valid_name_bad_labels",
        "uninit_metric", "writing_metric"}) {
    EXPECT_FALSE(metrics.counters.contains(name))
        << "Found unexpected counter: " << name;
  }

  for (absl::string_view name : {"nan_gauge", "inf_gauge"}) {
    EXPECT_FALSE(metrics.gauges.contains(name))
        << "Found unexpected gauge: " << name;
  }

  EXPECT_FALSE(metrics.histograms.contains("inf_hist"));
}
...
TEST_F(ShmCollectorTest, ResilientToCorruptFiles) {
  ...
  absl::Cleanup restore_permissions = [unreadable_path, is_root] {
    if (!is_root) {
...
TEST_F(ShmCollectorTest, IgnoresTmpFilesAndPreservesActiveWorkers) {
  ShmWriter live(WriterOptions("live"));
  live.IncrementCounter(metric_names::kSentBytesTotal, kPushLabels, 100);
  ...
  EXPECT_EQ(metrics.counters[metric_names::kSentBytesTotal]["direction=push"],
            100);
  EXPECT_TRUE(FileExists(dead_chunk));
...
TEST_F(ShmCollectorTest, SkipsUnlinkedSegmentUnderSharedLock) {
  ...
  if (pid == 0) {
    ...
    const AggregatedMetrics metrics = Collect();

    // Precondition: inotify / SIGIO handler must have executed.
    if (UnlinkContext::invoked.load(std::memory_order_acquire) != 1) {
      _exit(7);
    }

    // Precondition: file must be unlinked while holder_fd is open.
    struct stat holder_stat{};
    if (fstat(holder_fd, &holder_stat) != 0 || holder_stat.st_nlink != 0) {
      _exit(8);
    }

    // Assertion: metrics aggregation must have skipped unlinked segment and
    // only aggregated the live worker.
    if (!(metrics.counters.size() == 1 &&
          metrics.counters.contains(metric_names::kSentBytesTotal) &&
          metrics.counters.at(metric_names::kSentBytesTotal)
              .contains("direction=push") &&
          metrics.counters.at(metric_names::kSentBytesTotal)
                  .at("direction=push") == 100)) {
      _exit(9);
    }
