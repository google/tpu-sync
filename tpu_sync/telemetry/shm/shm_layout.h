...
// 4-byte magic signature identifying a valid Raiden POSIX shared-memory segment
// file. Written with release memory ordering to signal publication to readers.
...
struct alignas(64) ShmTocEntry {
  // Metric stream identifier. Writers must ensure null-termination within the
  // fixed buffer, and readers must read it as a bounded string (e.g.
  // absl::string_view or bounded length) to prevent reading past array
  // boundaries.
  char metric_name[64];
  // Encoded label key-value pairs. Writers must ensure null-termination within
  // the fixed buffer, and readers must read it as a bounded string (e.g.
  // absl::string_view or bounded length) to prevent reading past array
  // boundaries.
...
// non-atomic fields (pid, max_toc_entries, data_pool_offset, chunk_index),
// and finally publishes the segment by writing kRaidenShmMagic to
...
// exporter pipelines. Metrics configured with `custom_buckets` in
// `MetricMetadata` are not supported by this fixed layout and observations are
// binned using `kDefaultHistogramBuckets` to maintain fixed 192-byte standard
// slot indexing.
//
