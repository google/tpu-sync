...
TEST(ShmLayoutTest, HistogramSingleThreadObservation) {
  ...
  EXPECT_DOUBLE_EQ(slot.sample_sum.load(), 0.0);

  // Value 0.05 falls into bucket 0 (<= 0.1).
  slot.Observe(0.05);
  EXPECT_EQ(slot.sample_count.load(), 1);
  EXPECT_DOUBLE_EQ(slot.sample_sum.load(), 0.05);
  ...
  EXPECT_DOUBLE_EQ(slot.sample_sum.load(), 5.55);
  EXPECT_EQ(slot.bucket_counts[0].load(), 1);
  ...
  EXPECT_DOUBLE_EQ(slot.sample_sum.load(), 60005.55);
  EXPECT_EQ(slot.bucket_counts[0].load(), 1);
  EXPECT_EQ(slot.bucket_counts[6].load(), 1);
...
TEST(ShmLayoutTest, HistogramConcurrentObservations) {
  ...
  EXPECT_DOUBLE_EQ(slot.sample_sum.load(), kNumThreads * kNumIters * kObsValue);
...
TEST(ShmLayoutTest, FloatingPointPrecisionAndAbsorptionStress) {
  ...
  EXPECT_DOUBLE_EQ(normal_slot.sample_sum.load(), kNormalIters * 0.125);
}
