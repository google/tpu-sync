...
bool ShmWriter::AllocateNewChunk() const {
  ...
  int fd = open(tmp_path.c_str(),
                ...
                options_.file_mode);
  if (fd < 0) {
    LOG(ERROR) << "ShmWriter failed to open temporary file " << tmp_path << ": "
               << std::strerror(errno);
    return false;
  }

  if (ftruncate(fd, kSegmentTotalFileSize) != 0) {
    LOG(ERROR) << "ShmWriter failed to ftruncate file " << tmp_path << ": "
               << std::strerror(errno);
    close(fd);
  ...
  void* addr = mmap(nullptr, kSegmentTotalFileSize, PROT_READ | PROT_WRITE,
                    MAP_SHARED, fd, 0);
  if (addr == MAP_FAILED) {
    LOG(ERROR) << "ShmWriter failed to mmap file " << tmp_path << ": "
               << std::strerror(errno);
    close(fd);
    unlink(tmp_path.c_str());
    return false;
  ...
  if (flock(fd, LOCK_SH | LOCK_NB) != 0) {
    LOG(ERROR) << "ShmWriter failed to flock file " << tmp_path << ": "
               << std::strerror(errno);
  ...
  if (rename(tmp_path.c_str(), path.c_str()) != 0) {
    LOG(ERROR) << "ShmWriter failed to rename file from " << tmp_path << " to "
