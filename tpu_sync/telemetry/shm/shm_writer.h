...
class ShmWriter {
 ...
 private:
  // Owning key stored in the metric pointer caches (counter_cache_,
  // gauge_cache_, histogram_cache_). Allocates owning strings only on cache
  // misses when a new metric stream is first registered.
  struct MetricKey {
    std::string name;
  ...
  // Non-owning view borrowing string pointers and lengths. Used on the hot path
  // during steady-state metric recordings to look up cached slot pointers with
  ...
  // Transparent hasher enabling heterogeneous lookup in absl::flat_hash_map.
  // Allows querying the map with MetricKeyView without constructing an owning
  // MetricKey.
  ...
  // Transparent equality comparator enabling heterogeneous lookup between
  // MetricKey and MetricKeyView in absl::flat_hash_map.
  struct MetricKeyEq {
    using is_transparent = void;
  ...
  const auto& GetCache() const ABSL_SHARED_LOCKS_REQUIRED(mutex_) {
  ...
  template <typename SlotT>
  auto& GetMutableCache() const ABSL_EXCLUSIVE_LOCKS_REQUIRED(mutex_) {
    const auto& cache = GetCache<SlotT>();
    using MapType = std::remove_cv_t<std::remove_reference_t<decltype(cache)>>;
    return const_cast<MapType&>(cache);
  }

  template <typename SlotT, MetricType Type>
  SlotT* AllocateSlotAndPublishEntry(absl::string_view name,
                                     absl::string_view encoded_labels,
                                     MetricKey key) const
