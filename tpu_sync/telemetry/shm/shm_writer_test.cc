...
class MappedSegment {
 public:
  ...
  const ShmTocEntry* FindToc(
    ...
    char label_buffer[sizeof(ShmTocEntry::encoded_labels)];
    std::optional<absl::string_view> encoded =
        FormatShmLabelsToBuffer(labels, absl::MakeSpan(label_buffer));
...
// Validates header magic, pid, initial TOC limits, contiguity, and
...
// Consolidates counter, gauge, histogram mutations, label sorting, delimiter
...
TEST_F(ShmWriterTest, MetricRecordingAndLabelEncoding) {
  ...
  EXPECT_EQ(same_histogram->sample_count.load(), 1);
  EXPECT_THAT(same_histogram->sample_sum.load(), DoubleEq(42.0));
}

// Multi-threaded stress: 32 writer threads + 4 reader threads verifying
...
// Consolidates flock contention, mutation persistence, and release on
// process destruction.
TEST_F(ShmWriterTest, AdvisoryLockingAndCrossProcessLifecycle) {
  ...
  EXPECT_FALSE(std::filesystem::exists(path, exists_ec));
  ...
  if (pid == 0) {
    ...
    close(pipe_parent_to_child[1]);
    {
      ...
      char signal = '1';
      write(pipe_child_to_parent[1], &signal, 1);
      read(pipe_parent_to_child[0], &signal, 1);
    }
  ...
  char sync_buf;
  ASSERT_EQ(read(pipe_child_to_parent[0], &sync_buf, 1), 1);
  ...
  EXPECT_EQ(child_mapped.ReadCounter(metric_names::kSentBytesTotal,
                                     {&child_label, 1}),
            777);
  EXPECT_FALSE(can_lock_exclusive(child_mapped.file_path()));

  write(pipe_parent_to_child[1], "1", 1);
  close(pipe_child_to_parent[0]);
  close(pipe_parent_to_child[1]);
  ...
  std::error_code child_exists_ec;
  EXPECT_FALSE(
      std::filesystem::exists(child_mapped.file_path(), child_exists_ec));
  EXPECT_FALSE(child_exists_ec);
}

// Tests TOC slot overflow (1024), 64 KB data pool overflow (342 histograms),
// and 16 chunks limit.
...
// Consolidates 63/64-byte name boundaries, 127/128-byte label boundaries,
...
TEST_F(ShmWriterTest, InputValidationAndFailureResilience) {
  ...
  EXPECT_THAT(mapped.ReadHistogram("h_nan"), IsNull());
